...
Our main response variables were woman's \jdcmt{I don't think this work adds anything; behavioural} reported intention to cut their daughters in the daughter model and the mixed (daughter-persistence) model, and woman's attitude on whether FGC should be continued in the persistence model.  \jdcmt{You are describing just some predictors but it sounds like all; The predictors selected} \jdcmt{I don't know what is being conveyed here, can we lose it? based on literature (e.g., aforementioned above)} were woman's FGC status and beliefs about FGC benefits (see supp for the benefit questionnaire).  \jdcmt{This seems very wordy: The beliefs of FGC benefits were quantified using average score \cfcmt{pls confirm} to identify} the strength of benefits associating with fgc practice. We also included gender awareness (see WHERE for questions of gender awareness proxy) in our models to test if women’s self-awareness would moderate \cf{wording} their intention of FGC practice \cfonly{cites of feminist theory on fgc}. Other socio-demographic variables included were:  age, education, religion (see the list of religion recode at table xx in appendix; with a footnote on how we recoded it), marital status, work status, media use and residence (urban vs. rural) in addition to country.  Media use and gender awareness were scored.  The followings were treated as random variables:  cluster ID (villages) and ethnicity (see the list of ethnicity recode at table xx in appendix; with a footnote on how we recoded it).
...
\jdcmt{What happened to the text about what we are actually doing with 
...
\jdcmt{Did we do anything weird here with intercepts? Why do we have different intercepts with different models?} We subtracted respondent (-1) from the cluster when testing the community effect \cfjdml{Please rewrite this. Also, do we need to mention how we treat cluster with only one sample (if that happens).} \jdcmt{No, we don't have to worry about clusters with only one sample.}

\cfcmt{Do we explain conveyance of uncertainty and data sampling?}
\jdcmt{Can you explain this question?}
...
\jdcmt{Not sure what any of this is about. Our response are ordinal, and binary is an example of ordinal. I don't think there needs a big problem with using ethnicity as a random effect, or with explaining it. I kind of feel we are going in circles because you're not happy with it -- but do you have a solution?}
...
\jdcmt{Why on request? I always distrust that from others. Any objection to publishing the code?}

\subsection{Results}\label{results-1}

\cf{We need to show both isoplots and effect plots because some binary variables are not on the isoplots.}

Baseline socio-demographic and sample characteristics are shown in figure x.  The prevalence of FGC are 31.8\%, 88.6\%, 45.9\% and 91.0\%, and 15.5\%, 73.9\%, 20.6\% and 65.5\% of the samples thought FGC should be continued in Kenya, Mali, Nigeria and Sierra Leon accordingly. \jdcmt{This \P is inaesthetic. If the figure is not clear enough, we could have a table? Or else, we could just give ranges here?}
